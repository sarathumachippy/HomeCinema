(function (app) {
    "use strict";
    app.controller("asset.transactions.purchase.list.Ctrl", purchaseOrderCtrl);
    purchaseOrderCtrl.$inject = ['$scope', '$rootScope', 'membershipService', '$modalInstance', '$timeout', 'apiService', 'Notification', '$routeParams', '$location', 'constants'];

    function purchaseOrderCtrl($scope, $rootScope, membershipService, $modalInstance, $timeout, apiService, Notification, $routeParams, $location, constants) {
        ...
        function searchPurchaseList(page) {
            ...
            apiService.get('/api/purchase/search/', config,
            listCompleted,
            listFailure);
        }
