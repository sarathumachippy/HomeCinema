(function (app) {
    ...
    function writeOffCtrl($modal, $scope, $http, $rootScope, $filter, $timeout, membershipService, apiService, Notification, $routeParams, $location, constants, commonFunctions) {
        ...
        function showWriteOffRecords() {
            ...
            $modal.open({
                templateUrl: 'scripts/spa/QBSAdmin/transactions/writeOff/writeOffList.html',
        ...
        function addUpdateItem() {
            ...
            if ($scope.depreciationMonthDiff != 0 && $scope.writeOff.Ast_UsefulLife != 0)
                $scope.ValueTran.AsvTr_Dep_Value = ($scope.purchRate * $scope.itemLst.length / $scope.writeOff.Ast_UsefulLife) / $scope.depreciationMonthDiff;
            else
                $scope.ValueTran.AsvTr_Dep_Value = 0.000;
