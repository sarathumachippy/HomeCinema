(function (app) {
    "use strict";
    app.controller("asset.transactions.assetTransfer.Ctrl", assetTrnsferCtrl);
    assetTrnsferCtrl.$inject = ['$modal', '$scope', '$http', '$rootScope', '$filter', '$timeout', 'membershipService', 'apiService', 'Notification', '$routeParams', '$location', 'constants', 'commonFunctions'];

    function assetTrnsferCtrl($modal, $scope, $http, $rootScope, $filter, $timeout, membershipService, apiService, Notification, $routeParams, $location, constants, commonFunctions) {
        ...
        function showTrnasferRecords() {
            $modal.open({
                templateUrl: 'scripts/spa/QBSAdmin/transactions/AssetTransfer/assetTransferList.html',
        ...
        function getLocId() {
            ...
            $scope.detailsLst = [];
            $scope.loacation = {};
            if ($scope.assetLoc != undefined)
                $scope.assetLoc.OldLoc_Name = '';

            apiService.get('/api/assetTransfer/GetLocId/' + $scope.item.Ast_Id + "/" + $scope.assetDepartment.OldDept_Cd + "/"
                , null,
           getLocComplete,
           getLocFailed);
        ...
        function getLocComplete(response) {
            $scope.loacation = response.data.LocationList;

        }

        function getLocFailed() { }
        ...
        $scope.addUpdateItem = function () {
            ...
            if ($scope.items != undefined && $scope.items.Ds_Id != undefined) {
                if ($scope.transMode == 'update') {
                    ...
                    // if ($scope.detailsLst.length == 1) {
                    //     Notification.error('Canot be add more than one list');
                    //     return;
                    // }
                    ...
                    for (var i = 0; i < $scope.detailsLst.length; i++) {
                        ...
                        if ($scope.detailsLst[i].Ast_SlNo == $scope.assetTrf.Ast_SlNo &&
                           $scope.detailsLst[i].Ast_Item_Ctr == $scope.assetTrf.Ast_Item_Ctr) {
                            Notification.error({ message: 'Duplicate item cannot be added!!', delay: 1000 });
                ...
                Notification.error({ message: 'Please enter the details', delay: 1000 });
        ...
        function SaveTransfer() {
            ...
            if ($scope.item.Ast_Id == undefined || $scope.item.Ast_Id == '') {
        ...
        $scope.headerCustLocation = [
         ...
         { title: 'Name', property: 'CustLoc_Name', show: true, width: '100px', type: 'value' }
        ]
