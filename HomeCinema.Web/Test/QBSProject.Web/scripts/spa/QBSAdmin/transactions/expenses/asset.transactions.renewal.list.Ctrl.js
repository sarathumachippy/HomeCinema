(function (app) {
    "use strict";
    app.controller("asset.transactions.renewal.list.Ctrl", renewalListCtrl);
    renewalListCtrl.$inject = ['$scope', '$rootScope', 'membershipService', '$modalInstance', '$timeout', 'apiService', 'Notification', '$routeParams', '$location', 'constants'];

    function renewalListCtrl($scope, $rootScope, membershipService, $modalInstance, $timeout, apiService, Notification, $routeParams, $location, constants) {
        ...
        function searchPOList(page) {
            ...
            apiService.get('/api/renewal/search/', config,
            listCompleted,
            listFailure);
        }
